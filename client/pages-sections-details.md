...
Для подробного разбора клиентских модулей см. связанные документы:

| Документ | Назначение |
| -------- | ---------- |
| [client-module-overview.md](client-module-overview.md) | Карта Client Module Dive |
| [client-page-lifecycle.md](client-page-lifecycle.md) | `BasePageV2`, lifecycle, save, validation |
| [client-section-detail-patterns.md](client-section-detail-patterns.md) | `BaseSectionV2`, `BaseDetailV2`, grid/detail update |
| [client-diff-attributes-rules.md](client-diff-attributes-rules.md) | `diff`, `attributes`, `rules`, validation |
| [client-sandbox-messages.md](client-sandbox-messages.md) | sandbox messages между page/detail/section |
| [client-service-calls.md](client-service-calls.md) | вызовы сервисов и client ESQ |
| [client-pattern-catalog.md](client-pattern-catalog.md) | реальные примеры из `Autogenerated/Src` |
| [client-troubleshooting.md](client-troubleshooting.md) | диагностика UI-проблем |
...
### 5. Профильные расширения OCC и Sender

В базовом решении страницы и секции OCC/Sender строятся на тех же базовых классах, но активно используют:

- сервисные вызовы через `ServiceHelper`;
- виртуальные атрибуты для переключения видимости полей;
- `sandbox`-сообщения для синхронизации детали и карточки;
- отдельные popup-страницы настройки cron expression.

Типовые примеры:

- `BPMSoftOCCChannelPage` и channel view-модули — настройка каналов OCC;
- `BPMSoftOCCChat1Page`, `BPMSoftOCCChat1Section`, `BPMSoftOCCChatMessageDetail` — операторский UI чатов;
- `BPMSoftOCCDelivery1Page` — карточка рассылки Sender;
- `RecipicentDetailSchema`, `RecipicentGroupDetailSchema` — детали получателей рассылки.

Подробности по этим сценариям вынесены в профильные документы:
