...
Этот документ остаётся кратким обзором WCF-сервисов. Для детального изучения используйте небольшой пакет связанных документов:

| Документ | Что раскрывает |
| -------- | -------------- |
| [services-overview.md](services-overview.md) | карта пакета, основные варианты сервисов и клиентских вызовов |
| [services-contracts-routing.md](services-contracts-routing.md) | `[ServiceContract]`, `[WebInvoke]`, `BodyStyle`, `DefaultServiceRoute`, `SspServiceRoute` |
| [services-response-errors.md](services-response-errors.md) | `ConfigurationServiceResponse`, DTO, primitive/string responses, error model |
| [services-userconnection-security.md](services-userconnection-security.md) | `UserConnection`, `SystemUserConnection`, `DBSecurityEngine`, лицензии |
| [services-client-calls.md](services-client-calls.md) | `ServiceHelper`, `this.callService`, `AjaxProvider`, wrapped result |
| [services-outgoing-rest.md](services-outgoing-rest.md) | исходящие REST/API вызовы и интеграционные границы |
| [services-pattern-catalog.md](services-pattern-catalog.md) | каталог рабочих service-паттернов из `Autogenerated/Src` |
| [services-troubleshooting.md](services-troubleshooting.md) | диагностика 404/500, `BodyStyle`, прав и response shape |
...
| BSSchedulerService | BPMSoftSender | Планировщик задач и расписаний рассылки |
| BPMSoftOCCChatService | BPMSoftOCC | Операторские чаты, сообщения, статусы, transfer |
| BPMSoftOCCChatConfigService | BPMSoftOCC | Отправка OCC-конфигурации в connector |
| BPMSoftOCCChatRequestService | BPMSoftOCC | Приём callback/request payload и статусов сообщений |
| BPMSoftOCCAddChannelService | BPMSoftOCC | Регистрация каналов и webhook-интеграций |
| BPMSoftOCCSetupService | BPMSoftOCC | Мастер настройки OCC |
| BPMSoftOCCTransferService | BPMSoftOCC | Передача чатов между группами/операторами |
| BPMSoftOCCAddMfmsJsonService | BPMSoftOCCWAMfmsJson | WhatsApp MFMS/Edna канал |
| BSDeliveryService | BPMSoftSender | Старт/остановка рассылки |
| RecipientGroupService | BPMSoftSender | Группы получателей рассылки |
...
- [Профильные OCC/Sender сервисы](bpmsoft-occ-services.md)
