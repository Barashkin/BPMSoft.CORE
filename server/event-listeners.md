...
Этот документ остаётся обзором. Детальный пакет EventListener Dive разбит на небольшие связанные документы:

| Документ | Что раскрывает |
| -------- | -------------- |
| [event-listeners-overview.md](event-listeners-overview.md) | карта пакета, границы Entity/App listener и EventsProcess |
| [event-listeners-app-lifecycle.md](event-listeners-app-lifecycle.md) | `IAppEventListener`, `AppEventListenerBase`, `OnAppStart`, `OnAppEnd` |
| [event-listeners-entity-hooks.md](event-listeners-entity-hooks.md) | entity hooks, before/after events, changed columns |
| [event-listeners-eventsprocess.md](event-listeners-eventsprocess.md) | generated `EventsProcess`, `ThrowEvent`, `OnExecuted`, `ScriptTask` |
| [event-listeners-validation-and-safety.md](event-listeners-validation-and-safety.md) | `e.IsCanceled`, validation, security, recursion risks |
| [event-listeners-async-and-jobs.md](event-listeners-async-and-jobs.md) | async executors, Quartz jobs, self-healing registration |
...
| [event-listeners-troubleshooting.md](event-listeners-troubleshooting.md) | диагностика listener'ов, EventsProcess, performance |
...
### Примеры App EventListener из базового решения
